...
public class Test11_If {
    ...
    private static Object eval(Object exp, Env env) {
        ...
        if (op.equals("lambda")) {
            // (lambda (w h) (* w h))
            List<String> params = (List<String>) tmp.get(1);
            Object body = tmp.get(2);
            Env origEnv = env;
            return new Func() {
                @Override
                public Object run(List<Object> args) {
                    ...
                    for (int i = 0; i < args.size(); i++) {
                        newEnv.put(params.get(i), args.get(i));
                    }
        ...
        if (op.equals("if")) {
            // (if n non-zero zero)
            return ((boolean) eval(tmp.get(1), env)) ? eval(tmp.get(2), env) : eval(tmp.get(3), env);
        }
        if (op.equals("eq")) {
            // (eq x y)
            return eval(tmp.get(1), env) == eval(tmp.get(2), env);
        }
